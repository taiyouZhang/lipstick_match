...
def data_operate(meanColor):
    with open('../data/lipstick.json', 'r', encoding='utf-8') as f:
      ret_dic = json.load(f)
    ...
    #the third loop covert value_array to RGB_array#
      ...
      for i in range(sum):
          ...
          G=RGB_array[i][1]
          B=RGB_array[i][2]
          RGB_temp[i]=abs(meanColor[0]-R)+abs(meanColor[1]*3/4-G)+abs(meanColor[2]-B)
...
if __name__ == '__main__':
     ...
     list=[]
     color_dir="../data/pic/test"
     count=lipcolor.load_color(color_dir,list)
     meanColor = lipcolor.Mean_color(count,list)
     print("the extracted RGB value of the color is {0}".format())
     #operate the data#
     data_operate(meanColor)
